...
const Register = () => {
  ...
  const handlesubmit = async (e) => {
    ...
    try {
      ...
      const result = await axios.post("http://localhost:5050/api/user/signup",formdata)
      ...
      console.log("error in submit function in register page",error)
    }
  }
  const gotoLogin = () => {
    Navigate('login')
  ...
  return (
    <div className='bg-[#272525] h-screen w-screen flex flex-col justify-center items-center'>
      ...
      <form onSubmit={handlesubmit}>
        <span className="block text-3xl mt-10 ml-10 text-[#2D050A]">REGISTER</span>
        ...
        />
        <button type="submit" className='text-2xl mt-10 ml-10 bg-[#B4B985] px-20 py-2 rounded-3xl hover:bg-amber-300 active:bg-blue-500'>REGISTER</button>
        <button onClick={gotoLogin} className='text-2xl mt-10 ml-10 bg-[#85B992] px-15 py-2 rounded-3xl hover:bg-amber-300 active:bg-blue-500'>GO TO LOGIN</button>
